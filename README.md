...
1. Radarr or Sonarr imports a completed download into the media library.
2. Trimarr detects the imported media through the configured connection.
3. The assigned flow determines which tracks are kept, removed, converted or extracted.
4. Trimarr processes the media using the selected CPU or hardware encoder.
5. The optimized output replaces the original library copy when processing completes successfully.
6. Progress, logs and before-and-after details remain available in the Trimarr interface.
...
- Hardware encoders such as NVIDIA NVENC, Intel Quick Sync Video (QSV), AMD AMF or other supported encoders
